...
*Guard contract (SO_RCVBUF half shipped ahead of Stage 4): the cap default lives in `recorder.DEFAULT_LIVE_RECORD_MAX_BYTES` (512 MiB), and the CLI tests in `tests/test_live_hardening.py` are gated by `pytest.importorskip("sceptre_pipeline.__main__")` so they self-activate when Stage 4 lands. Stage 4's `__main__.py` must therefore expose `build_parser()` and default `--record-max-bytes` to `recorder.DEFAULT_LIVE_RECORD_MAX_BYTES`.*
